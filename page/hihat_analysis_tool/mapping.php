...
*/
    ...
    //@mysqli_connect(MYSQL_HOST, MYSQL_USER, MYSQL_PASS) OR die(mysql_error());
    ...
    if ( isset( $_POST['queryValues'] )){
        $conn = @mysqli_connect(MYSQL_HOST, MYSQL_USER, MYSQL_PASS, MYSQL_DATABASE) OR die(mysqli_error($conn));
        ...
        if ( $searchMode == 1 && $searchEngine->addQuery() == "WHERE ID IN (-1)" ) { 
                if ( !IPmapper::is_private_ip( $searchEngine->getSearchString() )) {     // if not private IP: add location
                    $conn = @mysqli_connect(MYSQL_HOST, MYSQL_USER, MYSQL_PASS, MYSQL_IPLOOKUP_DB) OR die(mysqli_error($conn));
              ...
              if ( $searchMode >= 1 && $searchMode <= 7 && $searchMode != 6 ) {
                ...
                if ( mysqli_num_rows( $result_serv )) {         
                      while($row = mysqli_fetch_assoc( $result_serv )) {   // look up IPs for the required ids
