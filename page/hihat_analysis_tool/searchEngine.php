      ...
      class SearchEngine { 
          ...
          function __construct( $searchStr, $active = "on" ) {                 
                ...
                if ( $active != "off")   
                    ...
                    } else if ( preg_match( "/^[0-9]+$/", $searchStr )) {                       // look for single ID
                        $this->mode = 3;
                        $this->searchString = (int)$searchStr;                       
                    } else if ( preg_match( "/^[0-9]+-[0-9]+$/", $searchStr )) {                // look for ID-range
                        $this->mode = 4;
                        $this->searchStart = (int)substr( $searchStr, 0, strpos( $searchStr, "-") );
                        $this->searchEnd   = (int)substr( $searchStr, strpos( $searchStr, "-") + 1 );                                                                
                    } else if ( preg_match( "/^[0-9]+((\ )*\,(\ )*[0-9]+)+$/", $searchStr )) { // look for seperated IDs
                        $this->mode = 5;
