...
*/
    ...
    $spiderTypes = array();   // array key: string with spider name, value: counter of its hits
    ...
    $referrerTypes = array();    // array key: string with HTTP_REFERER, value: counter of its hits, only set for known search engines
    $referrerEngines = array();  // array key: string with name of search engine used in referrer, value: counter of hits
    ...
    $toolFiletypes = array();    // array key: string with mime-filetypes of downloaded tools, value: counter of its hits
    ...
    $sql = "SELECT ID, attackerIP, attackerBrowser, Source, Value_Server, Value_Get, 
                  ...
                  ORDER BY
                        ID ASC";         
     $result_serv = mysqli_query($conn, $sql) OR die(mysqli_error($conn));
     
    //echo      $searchEngine->addQuery();     
    
    // go through server_vars
   if ( $totalHits = mysqli_num_rows( $result_serv ) ) {         
    ...
    if ( $totalToolsDownloaded = mysqli_num_rows( $result )) {                    
