        ...
        private function checkSingleString( $downloadLink, $toolStr ){
            ...
            if ( $foundTool ){
                ...
                if ( !(strcmp( $toolStr, "http://") == 0 || strcmp( $toolStr, "https://") == 0) )   // if no tool is looked for don't cut of beginning
                    $tempLink = substr( $downloadLink, strpos( $downloadLink, $toolStr ) + strlen($toolStr)); // cut off everything after first "WGET"
                else {    // if no tool used but http:// is looked for   
                    if ( (strpos( $tempLink, "http://") === false) && (strpos( $tempLink, "https://") === false ))     
                        return 0;    
                }                                                                                 
                $tempLink = preg_replace("/^[^a-zA-Z]*/" , "", $tempLink );    // remove starting spaces, %20 etc.                
                    ...
                    if($remotefile) {         
                        ...
                        if ($buffer != "" ) { 
                                ...
                                if (!$duplicate) {
                                    ...
                                    $writeResult = @mysqli_query($conn, $sql) OR die(mysqli_error($conn)); 
                                    if ($writeResult) {                                      
