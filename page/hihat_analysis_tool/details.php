...
*/
   ...
   if ( isset( $_SESSION['sMode'] )){   
         ...
         $sIdArray = isset($_SESSION['sIdArray']) ? $_SESSION['sIdArray'] : array() ; 
        ...
        // if Session active: determine next + previous Index for paging (bl�ttern)
        $sql = "SELECT Max(ID) as ID FROM main_logs              
                ".$searchEngine->addQuery()." AND ID < ".$id;        //echo $sql."<br> ";       
        $result = mysqli_query($conn, $sql) OR die(mysqli_error($conn));               
        $nextLowest = mysqli_fetch_row($result);
        ...
        $sql = "SELECT Min(ID) as ID FROM main_logs              
               ".$searchEngine->addQuery()." AND ID > ".$id;        //echo $sql."<br> "; 
        $result = mysqli_query($conn, $sql) OR die(mysqli_error($conn));               
        $nextHighest = mysqli_fetch_row($result);
   ...
   if ( mysqli_num_rows( $result )) {         
    ...
    //$auto = new autoDownloader(MAX_FILESIZE, 444, "pma_username = wget http://www.gmx.de/index.php;ajkl�fetch%20http://www.web.de/index.php?jkls=ka");
